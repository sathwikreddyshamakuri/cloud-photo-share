...
export default function AlbumsPage() {
  ...
  async function handleRename(e: FormEvent) {
    ...
    try {
      ...
      setAlbums(upd); setFiltered(upd); setRenaming(null);
  ...
  return (
    <div className="p-8 bg-slate-50 dark:bg-slate-800 min-h-screen text-slate-900 dark:text-slate-100">
        ...
        {filtered.map(alb => (
          <div key={alb.album_id}
            ...
            <Link to={`/albums/${alb.album_id}`}>
              ...
              <div className="p-4"><h2 className="text-lg font-medium">{alb.title}</h2></div>
            </Link>
      ...
      {/* modals (create & rename) … unchanged from previous code */}
      {/* … keep your existing modal markup here … */}
    </div>
  );
}
